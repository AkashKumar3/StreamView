...
import { AiOutlineCamera } from "react-icons/ai";  // New video upload icon
import { FiBell } from "react-icons/fi";  // New bell icon
            ...
            {userChannel && Object.keys(userChannel).length >= 1 ? (
              <>
                <Link className="hidden sm:block" to={"/uploadVideo"}>
                  <AiOutlineCamera size={20} /> {/* Changed video upload icon */}
                </Link>
              </>
            ) : (
              ""
            )}
            <FiBell className="hidden sm:block" size={20} /> {/* Changed bell icon */}
